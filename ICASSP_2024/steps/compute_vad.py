...
def vad_main(path):
    ...
    vad_audio = b''.join([segment for segment in segments])
    if len(vad_audio):
        write_wave(vad_path, vad_audio, sample_rate)

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', help='data dir', type=str, default="data")
    ...
    args = parser.parse_args()
    outSequences, _ = findAllSeqs(args.data_dir, args.extension, args.speaker_level)
