...
def cli_main(config='config.yaml', **kwargs):
    ...
    if configs.get('evaluate', False) is not True:
        ...
        configs['default_root_dir'] = configs.get('exp_dir', 'exp') + '/' + 'checkpoints' + '/' + configs['consistency_loss']['loss_a_type']
        ...
        if configs.get('lmft', False):
            ...
            if torch.cuda.is_available():
                state_dict = torch.load(configs['checkpoint_path'])
            else:
