...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 4, embedding_dim=1000):
        self.inplanes = 64
...
def Speaker_Encoder(embedding_dim=256, **kwargs):
    model = res2net50(embedding_dim=embedding_dim)
    return model

if __name__ == '__main__':
    # Input size: batch_size * seq_len * feat_dim
    x = torch.zeros(2, 80, 200)
    model = Speaker_Encoder(embedding_dim=128)
    print(model)
    out = model(x)
