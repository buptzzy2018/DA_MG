...
class CAMPPlus(nn.Module):
    def __init__(self,
        ...
        self.head = FCM(block=BasicResBlock, num_blocks=[2, 2], feat_dim=n_mels)
...
def Speaker_Encoder(embedding_dim=512, **kwargs):
    model = CAMPPlus(embedding_dim=embedding_dim, **kwargs)
    return model


if __name__ == '__main__':
    x = torch.zeros(10, 80, 200) # (B, F, T)
    model = CAMPPlus(n_mels=80, embedding_dim=512, pooling_type='TSP')
    model.eval()
    out = model(x)
    print(out.shape)
