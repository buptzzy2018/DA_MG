...
class Train_Dataset(Dataset):
    def __init__(self, data_list_path, aug_prob, speed_perturb, max_frames, sample_rate, 
    ...
    def __getitem__(self, indices):
        ...
        if type(indices) == int:
            audio, sr = loadWAV(self.data_list[indices], self.max_frames)
            genre = self.genre_list[indices]
            print('genre',genre)
            genre_feat.append(genre)
            return torch.FloatTensor(audio), self.data_label[indices],torch.tensor(genre_feat)
