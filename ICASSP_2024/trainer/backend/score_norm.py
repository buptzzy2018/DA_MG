...
def main(score_norm_method,
    ...
    _, enroll_list, test_list, _ = zip(*read_table(trial_score_file))
    enroll_list = sorted(list(set(enroll_list)))  # remove overlap and sort
    test_list = sorted(list(set(test_list)))

    enroll_emb, enroll_utt2idx = split_embedding(enroll_list, eval_emb_scp, mean_vec)
    ...
    np.save(score_norm_file[:-4] + '_enroll_cohort_mean.npy', enroll_mean)
    np.save(score_norm_file[:-4] + '_test_cohort_mean.npy', test_mean)
    with open(score_norm_file[:-4] + '_enroll_utt2idx.json', 'w') as f:
        f.write(json.dumps(enroll_utt2idx, indent=2))
    with open(score_norm_file[:-4] + '_test_utt2idx.json', 'w') as f:
        f.write(json.dumps(test_utt2idx, indent=2))
    ...
    with open(trial_score_file, 'r', encoding='utf-8') as fin:
        with open(score_norm_file, 'w', encoding='utf-8') as fout:
            ...
            for line in tqdm(lines):
                ...
                enroll_idx = enroll_utt2idx[line[1]]
                test_idx = test_utt2idx[line[2]]
                score = float(line[3])

                normed_score = 0.5 * (
                    (score - enroll_mean[enroll_idx]) / enroll_std[enroll_idx]
                    + (score - test_mean[test_idx]) / test_std[test_idx])
                fout.write('{} {} {} {}\n'.format(line[0], line[1], line[2], normed_score))
    logging.info("Over!")
