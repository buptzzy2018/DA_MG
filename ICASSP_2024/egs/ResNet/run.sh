...
config=conf/wbda.yaml
exp_dir=exp/resnet_asp_aam
ckpt_path=./checkpoint/avg_model.ckpt
cuda_device=4,5  #change --gpus correspondingly if you change the device
stage=4
stop_stage=4
...
if [ $stage -eq 6 ] ;then
  ...
  ckpt_path=$exp_dir/checkpoints/Baseline/avg_model.ckpt
  echo $ckpt_path
